...
# ######### Функции, связанные с симметрией. Соображения симметрии повышают производительность нахождения решения.######
...
#  #################### Перестановки и изменения ориентации основных симметрий ###################################
"""
120° clockwise rotation around the long diagonal URF-DBL.
Вращение на 120° по часовой стрелке вокруг длинной диагонали URF-DBL.
"""
...
Вращение на 90° по часовой стрелке вокруг оси через центры U и D.
"""
...
Отражение на плоскости через центры U, D, F, B.
"""
...
# ######################################## Fill SymCube list ###########################################################
# ################################## Заполнить мписок SymCube list #####################################################

"""
48 CubieCubes will represent the 48 cube symmetries
48 CubieCubes будут представлять 48 симметрий куба.
"""
symCube = []
...
Индексы обратной симметрии: SymCube [inv_idx [idx]] == SymCube [idx]^(- 1)
"""
inv_idx = [0] * N_SYM
...
# ################################## Создание групповой таблицы для 48 симметрий куба ##################################
...
# ########### Генерация таблицы сопряжения движения m симметрией s. conj_move[m, s] = s*m*s^-1 #########################
...
# ########### Генерация таблицы фазы 1 для сопряжения поворота t симметрией s. twist_conj[t, s] = s*t*s^-1 #############
fname = "conj_twist"
...
# #################### Генерация таблицы фазы 2 для сопряжения координаты URtoDB с помощью симметрии ################
fname = "conj_ud_edges"
if not path.isfile(fname):
	print("creating " + fname + " table...")
...
# ############## Генерация таблиц для обработки уменьшенной симметрии координаты flip-slice в фазе 1 ###################
fname1 = "fs_classidx"
fname2 = "fs_sym"
fname3 = "fs_rep"
...
# ########## Генерация таблиц для обработки координаты перестановки углов с уменьшенной симметрией в фазе 2. ###########
fname1 = "co_classidx"
fname2 = "co_sym"
fname3 = "co_rep"
if not (path.isfile(fname1) and path.isfile(fname2) and path.isfile(fname3)):
