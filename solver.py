...
# ########################### class SolverThread реализует двухфазный алгоритм #########################################
import face
...
class SolverThread(thr.Thread):
	...
	def __init__(self, cb_cube, rot, inv, ret_length, timeout, start_time, solutions, terminated, shortest_length):
		...
		:param cb_cube:	The cube to be solved in CubieCube representation
						Куб, который нужно решить, представленный в виде CubieCube
		:param rot:	Rotates the  cube 120° * rot along the long diagonal before applying the two-phase-algorithm
					Поворачивает куб на 120° * rot вокрук длинной диагонали (URF-DBL) перед применение двухфазного алгоритма
		:param inv:	0:	Do not invert the cube.	1:	Invert the cube before applying the two-phase-algorithm.
						Не отражает куб.			Отражает (L-R) куб перед применением двухфазного алгоритма.
		:param ret_length:	If a solution with length <= ret_length is found the search stops.
							Если найденная длина решения <= ret_length, то процесс поиска прекращается.
		 ...
		 Самый эффективный способ собрать куб - запустить шесть потоков параллельно с rot = 0, 1 и 2 и
		 inv = 0, 1. Первый поток, который находит решение, устанавливает флаг завершения, который сигнализирует всем остальным потокам.
		 закончить. В среднем это решает куб примерно в 12 раз быстрее, чем решение одного куба с помощью одного потока.
		 И это несмотря на использование Pythons GlobalInterpreterLock GIL.
		:param timeout:	Essentially the maximal search time in seconds. Essentially because the search does not return
		 before at least one solution has been found.
		 				По сути, максимальное время поиска в секундах. По сути, потому что поиск не возвращается,
		 пока не будет найдено хотя бы одно решение. 
		...
		:param solutions:	An array with the found solutions found by the six parallel threads.
							Массив с найденными решениями, найденными шестью параллельными потоками.
		:param terminated:	An event shared by the six threads to signal a termination request.
							Событие, разделяемое шестью потоками, чтобы сигнализировать о запросе на завершение.
		:param shortest_length:	The length of the shortes solutions in the solution array.
								Длина кратчайших решений в массиве решений.
		"""
		...
		эти переменные разделяются шестью потоками, инициализируемыми в функции решения.
		"""
	...
	def run(self):
		...
		итеративное углубление, решение имеет как минимум дальние ходы
		"""
...
def solve(cubestring, max_length=20, timeout=2):
	...
	:param timeout:	If the function times out, the best solution found so far is returned. If there has not been found
	 any solution yet the computation continues until a first solution appears.
	 				Если время ожидания функции истекает, возвращается лучшее решение, найденное на данный момент.
	 Если решение еще не найдено, вычисления продолжаются до тех пор, пока не появится первое решение.
	"""
	fc = face.FaceCube()
	...
	Эти изменяемые переменные модифицируются всеми шестью потоками.
	"""
	solutions = []
	...
	s = ''
	if len(solutions) > 0:
		print("Number of spins in the solution:", len(solutions[-1]))
		for m in solutions[-1]:  # the last solution is the shortest
			s += m.name + ' '
		return s.replace('3', "'").replace('1', '')
	else:
		print("cube ok")
		return ""
########################################################################################################################